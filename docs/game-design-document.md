...
- **Two kinds:** **footmen** (melee interceptors) and, in **fewer numbers, archers** (ranged, stay near the building). **Changed 2026-09-25: archers are castle-only — towers send footmen only** (2 footmen each); the castle keeps 6 footmen + 2 archers. Fewer unit types to balance and read.
...
- **Finite reserve (decided):** each building holds a fixed number of soldiers and releases them a couple at a time while a monster is within its alert range. Soldiers that die are gone; there's no respawn. **First pass — tower:** 2 footmen + 1 archer, 2 out at a time. **Castle:** 6 footmen + 2 archers, 4 out at a time. If a building falls, its unreleased reserve is lost; soldiers already out keep fighting.
...
- **Effects per level (first pass):**
  ...
  - **The castle calls for reinforcements:** **+2 footmen** added to its reserve each level (`reinforcementsPerAlarmLevel`; towers 0). This is a deliberate exception to "no respawn" (§3 "Defenders"): time is the one thing that refills the reserve.
